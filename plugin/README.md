...
7. **🔍 Selección** — con algo seleccionado (mouse, Explorer, o multi-selección con Ctrl/Shift), sube su informe a `snapshots/seleccion_<timestamp>.json`; el log confirma con `✓ Subida: … — N instancia(s), M script(s)`. El highlight del cursor es del mundo 3D; la GUI 2D se elige en el Explorer. Si la selección está vacía, se usa el objeto bajo el cursor (v1.9.1).
