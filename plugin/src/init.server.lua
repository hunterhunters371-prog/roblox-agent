-- Roblox Agent Bridge — LOADER (v2.0).
...
-- Solo si cambia ESTE archivo hará falta reinstalar (raro; se avisará en el README).
--
-- Seguridad: el runtime se descarga del repo oficial por HTTPS (es público: lectura
-- sin token) y cada versión queda registrada en el historial git (auditable).
...
local function fetchText(relPath)
	-- el CDN de raw cachea unos minutos: el query lo evita
	local url = ("%s%s?t=%s"):format(RAW_BASE, relPath, tostring(os.clock()))
...
local function mostrarFallback(detalle)
	...
	label.Size = UDim2.new(1, -24, 0, 150)
	label.TextXAlignment = Enum.TextXAlignment.Left
	label.TextYAlignment = Enum.TextYAlignment.Top
	label.Text = "No se pudo descargar el runtime del Bridge.\n\n"
		.. tostring(detalle)
		.. "\n\nComprueba tu conexión y que el place tenga «Enable Studio Access to API Services» (Game Settings → Security), y pulsa Reintentar."
	label.Parent = frame
	...
	boton.Position = UDim2.new(0, 12, 0, 170)
	boton.Size = UDim2.new(0, 110, 0, 30)
	boton.Parent = frame
	boton.MouseButton1Click:Connect(function()
		task.spawn(actualizar)
	end)
	widget.Enabled = true
end
...
actualizar = function()
	...
	local ok, err = pcall(function()
		...
		if runtimeFolder and manifest.version == versionActual then
			setTitulo(nil) -- ya estás en la última versión
			return
		end
...
widget:GetPropertyChangedSignal("Enabled"):Connect(function()
	if widget.Enabled and os.clock() - ultimaComprobacion > RECHECK_SECONDS then
		task.spawn(actualizar)
	end
end)
